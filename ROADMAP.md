...
- [ ] Resolve the Biome config-vs-CLI version mismatch so `lint`/`format` are trustworthy again — **re-verified post-1.8.0:** CLI now 2.5.6 (bumped by #115), config schema still 2.4.13, `biome check .` reports ~1,624 errors. Needs a dedicated config-migration + fix-or-suppress pass.
...
| Need | Why | Urgency |
| --- | --- | --- |
| Review + merge daily PR #118 (post-release reconciliation: roadmap/changelog/release policy + security overrides incl. new `sharp` pin) | Last unmerged piece of Week 1; overrides don't protect anyone until merged | Medium |
