...
- PR #115 (`divyanshudhruv/core:main` → `main`, community) — re-reviewed 2026-07-29: both blocking items from the 07-28 review are fixed and verified locally (build/typecheck green, tests at the known-failure baseline). One new flag: a post-review commit drops the `require` condition from every `exports` subpath — a package-resolution change that pushes the release to a minor (see `RELEASING.md` criteria). Recommendation to Lorant in the PR thread.
- PR #117 (Prism grammar aliases) — green, merge authorized by Lorant's 07-29 note but explicitly gated on #115 getting his APPROVED review; merges as part of release assembly, not before.
