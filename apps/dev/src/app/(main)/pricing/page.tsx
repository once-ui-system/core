"use client";

import { useEffect, useRef, useState } from "react";
import {
  AccordionGroup,
  Background,
  BlobFx,
  Button,
  Column,
  CountFx,
  Grid,
  Heading,
  Icon,
  MatrixFx,
  RevealFx,
  Row,
  SegmentedControl,
  Table,
  Tag,
  Text,
  useInViewport,
} from "@once-ui-system/core";

type Billing = "monthly" | "yearly";

type Tier = {
  name: string;
  description: string;
  price?: { monthly: number; yearly: number };
  features: string[];
  cta: string;
  highlighted?: boolean;
};

const tiers: Tier[] = [
  {
    name: "Starter",
    description: "For solo builders shipping their first product.",
    price: { monthly: 19, yearly: 15 },
    features: ["3 projects", "5 team members", "Core analytics", "Community support"],
    cta: "Start with Starter",
  },
  {
    name: "Pro",
    description: "For growing teams that need speed and depth.",
    price: { monthly: 49, yearly: 39 },
    features: [
      "Unlimited projects",
      "25 team members",
      "Advanced analytics",
      "API access",
      "Priority support",
    ],
    cta: "Start free trial",
    highlighted: true,
  },
  {
    name: "Enterprise",
    description: "For organizations with security and scale requirements.",
    features: [
      "Unlimited everything",
      "SSO / SAML",
      "Audit logs",
      "Dedicated success manager",
      "99.99% uptime SLA",
    ],
    cta: "Talk to sales",
  },
];

const included = (
  <Icon name="checkbox" size="xs" onBackground="brand-medium" decorative={false} aria-label="Included" />
);
const excluded = (
  <Icon name="minus" size="xs" onBackground="neutral-weak" decorative={false} aria-label="Not included" />
);

const comparisonData = {
  headers: [
    { content: "Feature", key: "feature" },
    { content: "Starter", key: "starter" },
    { content: "Pro", key: "pro" },
    { content: "Enterprise", key: "enterprise" },
  ],
  rows: [
    ["Projects", "3", "Unlimited", "Unlimited"],
    ["Team members", "5", "25", "Custom"],
    ["Core analytics", included, included, included],
    ["Advanced analytics", excluded, included, included],
    ["API access", excluded, included, included],
    ["SSO / SAML", excluded, excluded, included],
    ["Audit logs", excluded, included, included],
    ["Priority support", excluded, included, included],
    ["Dedicated success manager", excluded, excluded, included],
  ],
};

const faqItems = [
  {
    title: "Can I switch plans later?",
    content: (
      <Text variant="body-default-s" onBackground="neutral-weak">
        Yes. Upgrades apply immediately and we prorate the difference; downgrades take effect at
        the start of your next billing cycle.
      </Text>
    ),
  },
  {
    title: "What happens when my trial ends?",
    content: (
      <Text variant="body-default-s" onBackground="neutral-weak">
        Your workspace stays intact in read-only mode. Add a payment method any time to pick up
        exactly where you left off — nothing gets deleted.
      </Text>
    ),
  },
  {
    title: "How does yearly billing work?",
    content: (
      <Text variant="body-default-s" onBackground="neutral-weak">
        Yearly plans are billed once per year at a 20% discount. You can switch from monthly to
        yearly at any point and the remaining balance is credited.
      </Text>
    ),
  },
  {
    title: "Do you offer discounts for startups or non-profits?",
    content: (
      <Text variant="body-default-s" onBackground="neutral-weak">
        Early-stage startups and registered non-profits get 50% off Pro for the first year. Reach
        out to sales with a short note about what you're building.
      </Text>
    ),
  },
  {
    title: "What payment methods do you accept?",
    content: (
      <Text variant="body-default-s" onBackground="neutral-weak">
        All major credit cards, plus invoicing with NET-30 terms on Enterprise plans.
      </Text>
    ),
  },
];

export default function PricingPage() {
  const [billing, setBilling] = useState<Billing>("monthly");

  const tiersRef = useRef<HTMLDivElement>(null);
  const tiersInViewport = useInViewport(tiersRef);
  const [tiersSeen, setTiersSeen] = useState(false);
  useEffect(() => {
    if (tiersInViewport) setTiersSeen(true);
  }, [tiersInViewport]);

  return (
    <Column as="main" fillWidth horizontal="center" gap="104" paddingBottom="104">
      {/* Hero — MatrixFx strip faded into the page */}
      <Column fillWidth center overflow="hidden" paddingY="80">
        <MatrixFx
          position="absolute"
          top="0"
          left="0"
          fill
          flicker
          size={1.5}
          spacing={8}
          colors={["brand-solid-strong"]}
          bulge={{ type: "wave", duration: 3, intensity: 20, repeat: true }}
        />
        <Background
          position="absolute"
          top="0"
          left="0"
          fill
          pointerEvents="none"
          gradient={{ display: true, colorStart: "page-background", x: 0, y: 50, width: 150, height: 300 }}
        />
        <Column zIndex={1} center gap="24" padding="48" maxWidth={48}>
          <RevealFx translateY="16">
            <Column center gap="16">
              <Text variant="label-default-s" onBackground="brand-medium">
                Pricing
              </Text>
              <Heading as="h1" variant="display-strong-xl" align="center">
                Pay for momentum, not seats
              </Heading>
            </Column>
          </RevealFx>
          <RevealFx delay={0.1} translateY="16">
            <Text variant="body-default-l" onBackground="neutral-weak" align="center">
              Start free, scale when you're ready. Every plan includes unlimited viewers and our
              full component library.
            </Text>
          </RevealFx>
        </Column>
      </Column>

      {/* Tiers */}
      <Column fillWidth horizontal="center">
        <Column fillWidth maxWidth="l" gap="40">
          <Row
            fillWidth
            horizontal="between"
            vertical="end"
            gap="24"
            s={{ direction: "column", horizontal: "start" }}
          >
            <Column gap="12">
              <Text variant="label-default-s" onBackground="brand-medium">
                Plans
              </Text>
              <Heading as="h2" variant="display-strong-xs">
                Three plans, one decision
              </Heading>
            </Column>
            <SegmentedControl
              fillWidth={false}
              selected={billing}
              onToggle={(value) => setBilling(value as Billing)}
              buttons={[
                { value: "monthly", label: "Monthly" },
                { value: "yearly", label: "Yearly −20%" },
              ]}
            />
          </Row>

          <Grid ref={tiersRef} columns="3" gap="24" s={{ columns: 1 }}>
            {tiers.map((tier, index) => (
              <RevealFx key={tier.name} fill trigger={tiersSeen} translateY="8" delay={index * 0.1}>
                <Column
                  fill
                  background="surface"
                  border={tier.highlighted ? "brand-alpha-medium" : "neutral-alpha-weak"}
                  radius="l"
                  padding="32"
                  overflow="hidden"
                >
                  {tier.highlighted && (
                    <Background
                      position="absolute"
                      top="0"
                      left="0"
                      fill
                      pointerEvents="none"
                      gradient={{
                        display: true,
                        colorStart: "brand-alpha-medium",
                        colorEnd: "static-transparent",
                        x: 50,
                        y: 0,
                        width: 200,
                        height: 60,
                        opacity: 50,
                      }}
                    />
                  )}
                  <Column zIndex={1} fill gap="24">
                    <Row fillWidth horizontal="between" vertical="center" gap="8">
                      <Heading as="h3" variant="heading-strong-m">
                        {tier.name}
                      </Heading>
                      {tier.highlighted && <Tag variant="brand" size="s" label="Most popular" />}
                    </Row>
                    <Text variant="body-default-s" onBackground="neutral-weak">
                      {tier.description}
                    </Text>
                    {tier.price ? (
                      <Row vertical="end" gap="2">
                        <Text variant="display-strong-s">$</Text>
                        <CountFx
                          variant="display-strong-s"
                          value={tier.price[billing]}
                          effect="wheel"
                          speed={400}
                        />
                        <Text variant="label-default-s" onBackground="neutral-weak" paddingBottom="4" paddingLeft="4">
                          / mo
                        </Text>
                      </Row>
                    ) : (
                      <Text variant="display-strong-s">Custom</Text>
                    )}
                    <Column flex={1} gap="12">
                      {tier.features.map((feature) => (
                        <Row key={feature} gap="12" vertical="center">
                          <Icon name="checkbox" size="xs" onBackground="brand-medium" />
                          <Text variant="body-default-s">{feature}</Text>
                        </Row>
                      ))}
                    </Column>
                    <Button
                      fillWidth
                      variant={tier.highlighted ? "primary" : "secondary"}
                      arrowIcon={tier.highlighted}
                    >
                      {tier.cta}
                    </Button>
                  </Column>
                </Column>
              </RevealFx>
            ))}
          </Grid>
        </Column>
      </Column>

      {/* Feature comparison — tinted band for contrast */}
      <Column fillWidth horizontal="center" background="neutral-alpha-weak" paddingY="80" paddingX="24">
        <Column fillWidth maxWidth="l" gap="40">
          <Column gap="12">
            <Text variant="label-default-s" onBackground="brand-medium">
              Compare
            </Text>
            <Heading as="h2" variant="display-strong-xs">
              Every feature, side by side
            </Heading>
          </Column>
          <Table fillWidth data={comparisonData} />
        </Column>
      </Column>

      {/* FAQ — split layout */}
      <Column fillWidth horizontal="center">
        <Row fillWidth maxWidth="l" gap="40" s={{ direction: "column" }}>
          <Column gap="16" maxWidth={20}>
            <Text variant="label-default-s" onBackground="brand-medium">
              FAQ
            </Text>
            <Heading as="h2" variant="display-strong-xs">
              Questions, answered
            </Heading>
            <Text variant="body-default-m" onBackground="neutral-weak">
              Can't find what you're looking for? Our team replies within one business day.
            </Text>
            <Button variant="secondary" size="s" suffixIcon="arrowUpRight" href="mailto:sales@example.com">
              Talk to sales
            </Button>
          </Column>
          <Column flex={1}>
            <AccordionGroup items={faqItems} />
          </Column>
        </Row>
      </Column>

      {/* Final CTA — ambient blob panel */}
      <Column fillWidth horizontal="center">
        <Column
          fillWidth
          maxWidth="l"
          radius="xl"
          border="neutral-alpha-weak"
          overflow="hidden"
          padding="64"
          center
        >
          <BlobFx position="absolute" top="0" left="0" fill opacity={60} seed={2} />
          <Column zIndex={1} center gap="24" maxWidth={40}>
            <Heading as="h2" variant="display-strong-s" align="center">
              Ship your next launch on Pro
            </Heading>
            <Text variant="body-default-m" onBackground="neutral-weak" align="center">
              14-day free trial, no credit card required. Cancel anytime.
            </Text>
            <Row gap="12" s={{ direction: "column" }}>
              <Button arrowIcon>Start free trial</Button>
              <Button variant="secondary">Talk to sales</Button>
            </Row>
          </Column>
        </Column>
      </Column>
    </Column>
  );
}
