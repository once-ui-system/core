...
**Used in components:** [OGCard](/components/og-card)
...
function LinkThumbnail({ url }: { url: string }) {
  ...
  if (loading) return <Skeleton width={200} size={200} />;
...
<PropsTable
  content={[
    ["url", ["string", "null"], "-"],
    ["customFetchUrl", "string", "'/api/og/fetch'"],
    ["customProxyUrl", "string", "'/api/og/proxy'"]
  ]}
/>
