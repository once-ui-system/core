...
`size` takes the same tokens as [`Icon`](/components/icon). Match it to
...
[`Setting`](/components/setting) has a `description` prop for exactly
...
[`Setting`](/components/setting) renders an `InfoTip` for you when you
