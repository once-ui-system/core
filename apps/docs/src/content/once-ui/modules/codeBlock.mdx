...
Display Git-style diffs by setting the language to "diff". Numbering can be used from coordinates, also specified manually by setting the `startLineNumber` parameter. Numbering prioritizes as follows:
<Card
    border="neutral-alpha-medium"
    vertical="center"
    horizontal='center'
    ...
    radius="l"
    paddingX="16">
    coordinates > `startLineNumber` > 0 (fallback to 0 if not set)
</Card>
