...
Display Git-style diffs by setting the language to "diff". Numbering can be used from coordinates, also specified manually by setting the `startLineNumber` parameter. Numbering prioritizes as follows: `coordinates` > `startLineNumber` > `0` (fallback)
...
You can enable syntax highlighting by specifying a similar array as the language parameter: `[ “diff”, “typescript” ]`
...
Use `maxLines` to limit code height and show a "View code" overlay for expandable code blocks. `isCollapsible` must be enabled for the overlay to appear.
...
Use the `background` prop to change the CodeBlock's outer background. Combine with alpha tokens for transparency effects.
    ...
    codes={[
      {
        language: "tsx",
        label: "Custom background",
        code: "console.log('Styled code block');"
      }
    ]}
/>`
    }
...
Pass any color via `style.backgroundColor` &mdash; the button wrappers automatically pick it up.

<CodeBlock
  marginTop="16"
  marginBottom="24"
  style={{ backgroundColor: "rgba(255, 0, 128, 0.2)" }}
  codes={[
    {
      language: "tsx",
      label: "Magenta (50%)",
...
<PropsTable
  content={[
    ...
    ["background", "Colors | 'surface' | 'overlay' | 'page' | 'transparent'", "'surface'"],
    ["onInstanceChange", "(index: number) => void"],
    ["hideCode", "boolean", "false", "Render only the preview, without the code panel."],
    ["...flex"]
  ]}
/>
