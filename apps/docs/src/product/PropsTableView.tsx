 ...
 *
 * The fifth element marks a prop required. Pages written by hand rarely set
 * it; `PropsTable` fills it in from the generated spec.
 */
type PropData = [string, PropType?, string?, ReactNode?, boolean?];

interface PropsTableViewProps {
  content: PropData[];
...
function PropsTableView({ content, label = "Prop" }: PropsTableViewProps) {
  const data = {
    headers: [
    ...
    rows: content.map(([name, type, defaultValue, description, required]) => {
...
export { PropsTableView };
export type { PropData, PropType };
